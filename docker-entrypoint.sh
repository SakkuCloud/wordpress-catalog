...
EOPHP
    ...
    ## Codes added by Sina Hamedheidari (sinaha65@gmail.com)
    php_specific_env=(
	...
	for e in "${php_specific_env[@]}"; do
		file_env "$e"
		if [ -z "$haveCustomConfig" ] && [ -n "${!e}" ]; then
			haveCustomConfig=1
		fi
	done

	if [ "$haveCustomConfig" ]; then

	for php_param in "${php_specific_env[@]}"; do	    
		if [ -n "${!php_param}" ];
		then
	       sed -i "s/^\($php_param\).*/\1 $(eval echo = \${$php_param})/" /usr/local/etc/php/php.ini
		fi
	done
	...
	## END Codes added by Sina Hamedheidari	
